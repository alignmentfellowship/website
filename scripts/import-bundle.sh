...
# Old URLs are already published and in a submitted sitemap. A renamed piece must
# redirect, not 404 — vercel.json is the only place a static export can say so.
python3 - "$ROOT" <<'PYGEN'
import json, pathlib, sys
root = pathlib.Path(sys.argv[1])
b = json.loads((root / "content" / "bundle.json").read_text())
# Both forms. The site sets trailingSlash: true, so the published URL carried a
# trailing slash, but a shared link may not — and whether Vercel matches a redirect
# before or after it normalises the slash is not worth depending on.
redirects = []
for old, new in sorted(b.get("renames", {}).items()):
    for src in (f"/writings/{old}", f"/writings/{old}/"):
        redirects.append({"source": src, "destination": f"/writings/{new}/",
                          "permanent": True})
cfg_path = root / "vercel.json"
cfg = json.loads(cfg_path.read_text()) if cfg_path.exists() else {}
cfg["redirects"] = redirects
cfg_path.write_text(json.dumps(cfg, indent=2) + "\n")
print(f"vercel.json: {len(redirects)} redirect(s) from renamed pieces")
PYGEN
